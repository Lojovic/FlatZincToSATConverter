...
void Encoder::write_lit_definition_clauses_sat_subspace(LiteralPtr lit){
    ...
    for(auto clause : temp_clauses){
        ...
        sat_subspace_vars.back().insert("x" + to_string(lit_num));
        ...
        for(auto l : clause){
            ...
            sat_subspace_vars.back().insert("x" + to_string(lit_num1));
...
void Encoder::write_definition_clauses() {
    ...
    for(const auto& clause : definition_clauses){
        ...
        for(auto l : temp_clause){
            ...
            if(l->pol)
                sat_subspace << "x" << lit_num << " ";
            else
                sat_subspace << "(not x" << lit_num << ") ";
...
void Encoder::write_sat_constraint_clauses() {
    ...
    for(const auto& clause : sat_constraint_clauses){
        ...
        for(const auto& lit : clause){
            ...
            if(lit->pol)
                sat_constraints << "x" << lit_num << " ";
            else
                sat_constraints << "(not x" << lit_num << ") ";
            
            sat_constraints_vars.back().insert("x" + to_string(lit_num));
        }
        if(clause.size() > 1)
            sat_constraints << ")";
    ...
    sat_constraints << "---\n";
    sat_constraints_vars.push_back({});

    sat_constraint_clauses.clear();
}

void Encoder::write_sat_dom_clauses(CNF& clauses) {
        ...
        for(const auto& lit : clause){
            ...
            if(curr_var != curr_name){
                ...
                if(lit->type == LiteralType::ORDER){
                    ...
                    int lit_num = literal_to_num[{lit->type, lit->id, lit->val}];
                    if(lit->pol == true)
                        sat_subspace << "(= x" << lit_num << " true)" << endl;
                    else
                        sat_subspace << "(= x" << lit_num << " false)" << endl;  
            ...
            if(lit->pol){
                ...
                sat_dom << "(not x" << lit_num << ") ";
                sat_subspace << "(not x" << lit_num << ") ";
            }

            sat_subspace_vars.back().insert("x" + to_string(lit_num));
            sat_dom_vars.back().insert("x" + to_string(lit_num));
        }
        if(clause.size() > 1){
            sat_dom << ")";
            sat_subspace << ")";
...
void Encoder::generate_proof(){
    ...
    ifstream smt_dom_reader = ifstream("trivial_encoding_domains.smt2");
    ...
    while(getline(smt_dom_reader, line)){
    ...
    if(smt_dom_num == 1){
        ...
        if(smt_dom_num > 2)
            proof_file << "(and\n";
        for(int i = 1; i < smt_dom_num; i++)
            proof_file << "smt_dom" << i << "\n";
        if(smt_dom_num > 2)
            proof_file << ")\n";
        proof_file << ")" << endl;
    }
    ...
    ifstream sat_dom_reader = ifstream("sat_dom.smt2");
    if(std::filesystem::file_size("sat_dom.smt2") > 0)
        proof_file << "(define-fun sat_dom" << sat_dom_num++ << " () Bool\n(and\n"; 
 
    should_define_fun = false;
    while(getline(sat_dom_reader, line)){
    ...
    if(sat_dom_num == 1){
        ...
        if(sat_dom_num > 2)
            proof_file << "(and\n";
        for(int i = 1; i < sat_dom_num; i++)
            proof_file << "sat_dom" << i << "\n";
        if(sat_dom_num > 2)
            proof_file << ")\n";
        proof_file << ")" << endl;
    }
...
void Encoder::generate_proof2step(){
    ...
    string line;
    bool has_smt_dom_step1_clause = false;
    while(getline(smt_dom_step1_reader, line)){

        if(line != "---"){
            if(!has_smt_dom_step1_clause){
                proof_file << "(and\n";
                has_smt_dom_step1_clause = true;
            }
            proof_file << line << endl;
        }
    }
...
BasicVar* Encoder::encode_int_range_helper_variable(const int left, const int right, CNF& cnf_clauses, bool is2step_var) {
    ...
    if(export_proof){
        if(is2step_var){
            sat_dom_clauses2step.push_back(clause1);
            sat_dom_clauses2step.push_back(clause2);
        } else {
            sat_dom_clauses.push_back(clause1);
            sat_dom_clauses.push_back(clause2);
        }
    ...
    for(int i = left; i <= right; i++){
        ...
        if(export_proof){
            if(is2step_var)
                sat_dom_clauses2step.push_back(curr_clause);
            else
                sat_dom_clauses.push_back(curr_clause);
        }
...
void Encoder::encode_constraint(Constraint& constr, CNF& cnf_clauses) {
    ...
    if(export_proof){
        ...
        write_sat_dom_clauses(sat_dom_clauses);
...
void Encoder::encode_array_int_maximum(const BasicVar& m, const ArrayLiteral& x, CNF& cnf_clauses){
    ...
    if(export_proof){
        ...
        smt_constraints_vars.push_back({*m.name});
...
void Encoder::encode_int_div(const BasicVar& a, const BasicVar& b, const BasicVar& c, CNF& cnf_clauses){
    ...
    if(b_left == 0 && b_right == 0){
        if(export_proof){
            smt_constraints_vars.push_back({*a.name, *b.name, *c.name});
            trivial_encoding_constraints << "(define-fun smt_c" << next_constraint_num++ << " () Bool\n false\n)\n";
        }
    ...
    if(a_right < 0){
        ...
        LiteralPtr not_neg_a = make_literal(LiteralType::ORDER, a.id, false, -1);
        ...
        if(r_right > 0)
            cnf_clauses.push_back({not_neg_a, neg_r}); 
        else if(r_left > 0)
            cnf_clauses.push_back({not_neg_a});
        ...
        if(export_proof){
            ...
            if(r_right > 0)
                sat_constraint_clauses.push_back({not_neg_a, neg_r});
            else if(r_left > 0)
                sat_constraint_clauses.push_back({not_neg_a});
        }
    }
...
void Encoder::encode_int_mod(const BasicVar& a, const BasicVar& b, const BasicVar& c, CNF& cnf_clauses){
    ...
    if(export_proof){
        ...
        smt_constraints_vars.push_back({*a.name, *b.name, *c.name, *c_abs->name, *b_abs->name,
                                        *bp->name, *p->name});
        connection2step_vars.back().insert({*a.name, *b.name, *c.name, *c_abs->name, *b_abs->name,
                            *bp->name, *p->name});
        connection2step_vars.push_back({});
    }
    ...
    if(a_right < 0){
        ...
        if(c_right > 0)
            cnf_clauses.push_back({not_neg_a, neg_c}); 
        else if(c_left > 0)
            cnf_clauses.push_back({not_neg_a});
        ...
        if(export_proof){
            ...
            if(c_right > 0)
                sat_constraint_clauses.push_back({not_neg_a, neg_c});
            else if(c_left > 0)
                sat_constraint_clauses.push_back({not_neg_a});
        }
    }
...
void Encoder::encode_array_bool_xor(const ArrayLiteral& as, CNF& cnf_clauses){
    ...
    if(export_proof){
        ...
        trivial_encoding_constraints << "(define-fun smt_c" << next_constraint_num++ << " () Bool\n";
        if(as.empty()){
            trivial_encoding_constraints << "false\n)\n";
        } else {
    ...
    if(as.size() == 1){
        ...
        cnf_clauses.push_back({yes_var0});

        if(export_proof)
            sat_constraint_clauses.push_back({yes_var0});
    ...
    if(export_proof){
        ...
        auto set_literal_to_bv = [&](const SetLiteral& set_lit){
            ...
            if(holds_alternative<SetSetLiteral*>(set_lit)){
                ...
                int left = get<SetRangeLiteral*>(set_lit)->left;
                int right = get<SetRangeLiteral*>(set_lit)->right;
